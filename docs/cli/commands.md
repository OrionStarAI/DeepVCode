...
- **`/chat`**
  ...
  - **Sub-commands:**
    ...
    - **`delete`**
      - **Description:** Deletes saved conversation checkpoints. You can delete a specific checkpoint by tag or delete all checkpoints.
      - **Usage:** `/chat delete <tag>` or `/chat delete --all`
      - **Aliases:** `remove`, `rm`
...
- **`/stats`**
  ...
  - **Sub-commands:**
    - **`model`**:
      - **Description:** Show model-specific usage statistics. Can specify a particular model name for detailed stats of that model only.
      - **Usage:** `/stats model [model_name]`
      - **Auto-completion:** Tab completion available for model names that have been used in the current session.
    - **`tools`**:
...
- [**`/tools`**](../tools/index.md)
  - **Description:** Display a list of tools that are currently available within Gemini CLI. By default, shows brief descriptions for each tool to help you understand their purpose.
  - **Sub-commands:**
    - **`nodesc`** or **`nodescriptions`**:
      - **Description:** Hide tool descriptions, showing only the tool names for a more compact view.
    - **`desc`** or **`descriptions`** (legacy):
      - **Description:** Show detailed descriptions of each tool. This is now the default behavior.
