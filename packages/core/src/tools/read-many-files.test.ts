...
describe('ReadManyFilesTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should include images as inlineData parts if explicitly requested by extension', async () => {
      ...
      const result = await tool.execute(params, new AbortController().signal);
      const expectedImagePath = path.join(tempRootDir, 'image.png');
      expect(Array.isArray(result.llmContent)).toBe(true);
      const llmContentArray = result.llmContent as Array<any>;
      expect(llmContentArray).toHaveLength(2);
      expect(typeof llmContentArray[0]).toBe('string');
      expect(llmContentArray[0]).toMatch(new RegExp(`--- ${expectedImagePath.replace(/\\/g, '\\\\')} ---\\n\\n\\[MEDIA FILE: IMAGE - image\\.png \\(compression failed:`));
      expect(llmContentArray[1]).toEqual({
        inlineData: {
          data: Buffer.from([
    ...
    it('should include images as inlineData parts if explicitly requested by name', async () => {
      ...
      const result = await tool.execute(params, new AbortController().signal);
      const expectedImagePath = path.join(tempRootDir, 'myExactImage.png');
      expect(Array.isArray(result.llmContent)).toBe(true);
      const llmContentArray = result.llmContent as Array<any>;
      expect(llmContentArray).toHaveLength(2);
      expect(typeof llmContentArray[0]).toBe('string');
      expect(llmContentArray[0]).toMatch(new RegExp(`--- ${expectedImagePath.replace(/\\/g, '\\\\')} ---\\n\\n\\[MEDIA FILE: IMAGE - myExactImage\\.png \\(compression failed:`));
      expect(llmContentArray[1]).toEqual({
        inlineData: {
          data: Buffer.from([
            0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
    ...
    it('should include PDF files as inlineData parts if explicitly requested by extension', async () => {
      ...
      const result = await tool.execute(params, new AbortController().signal);
      const expectedPdfPath = path.join(tempRootDir, 'important.pdf');
      expect(result.llmContent).toEqual([
        `--- ${expectedPdfPath} ---\n\n[MEDIA FILE: PDF - important.pdf]\n\n`,
        {
          inlineData: {
    ...
    it('should include PDF files as inlineData parts if explicitly requested by name', async () => {
      ...
      const result = await tool.execute(params, new AbortController().signal);
      const expectedPdfPath = path.join(tempRootDir, 'report-final.pdf');
      expect(result.llmContent).toEqual([
        `--- ${expectedPdfPath} ---\n\n[MEDIA FILE: PDF - report-final.pdf]\n\n`,
        {
          inlineData: {
            data: Buffer.from('%PDF-1.4...').toString('base64'),
            mimeType: 'application/pdf',
