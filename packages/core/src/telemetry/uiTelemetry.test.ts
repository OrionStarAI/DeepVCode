...
describe('UiTelemetryService', () => {
  ...
  describe('API Response Event Processing', () => {
    ...
    it('should handle ApiResponseEvents for different models', () => {
      ...
      const metrics = service.getMetrics();
      expect(metrics.models['test-model-pro']).toBeDefined();
      expect(metrics.models['test-flash-model']).toBeDefined();
      expect(metrics.models['test-model-pro'].api.totalRequests).toBe(1);
      expect(metrics.models['test-flash-model'].api.totalRequests).toBe(1);
