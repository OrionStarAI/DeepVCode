...
describe('mcpCommand', () => {
  ...
  describe('no MCP servers configured', () => {
    ...
    it('should display help content when no MCP servers are configured', async () => {
      ...
      expect(result.content).toContain('未配置 MCP 服务器');
      expect(result.content).toContain('/mcp add');
      expect(result.content).toContain('/mcp help');
      expect(result.content).toContain('GitHub仓库操作工具');
      expect(result.content).toContain('SQLite数据库工具');
    });
  });
