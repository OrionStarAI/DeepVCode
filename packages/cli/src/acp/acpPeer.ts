...
import {
  ...
  // clearCachedCredentialFile - removed with OAuth2
  isNodeError,
  getErrorMessage,
  isWithinRoot,
  getErrorStatus,
...
import { Part, FunctionCall, PartListUnion } from '@google/genai';
...
class GeminiAgent implements Agent {
  ...
  async authenticate(): Promise<void> {
    ...
    // Auth refresh only needed for Google OAuth, not for Cheeth OA
    console.log('⚠️ Auth refresh not needed for Cheeth OA authentication');
  ...
  async sendUserMessage(params: acp.SendUserMessageParams): Promise<void> {
    ...
    let nextMessage: Content | null = { role: MESSAGE_ROLES.USER, parts };
    ...
    while (nextMessage !== null) {
      ...
      if (functionCalls.length > 0) {
        ...
        nextMessage = { role: MESSAGE_ROLES.USER, parts: toolResponseParts };
      }
    }
  }
...
function toToolCallContent(toolResult: ToolResult): acp.ToolCallContent | null {
  if (!toolResult.returnDisplay) return null;

  const rd: unknown = toolResult.returnDisplay as unknown;
  if (typeof rd === 'string') {
    return { type: 'markdown', markdown: rd };
  }

  // Handle structured TODO display by mapping it to markdown for ACP clients
  if (typeof rd === 'object' && rd !== null && (rd as any).type === 'todo_display') {
    const todo = rd as any;
    const title = todo.title || 'Todos';
    const lines: string[] = Array.isArray(todo.items)
      ? todo.items.map((it: any) => {
          const mark = it.status === 'completed' ? '[x]' : it.status === 'in_progress' ? '[·]' : '[ ]';
          return `- ${mark} ${it.content}`;
        })
      : [];
    return { type: 'markdown', markdown: `${title}\n${lines.join('\n')}` };
  }

  // Fallback to diff content
  const fileDiff = rd as { fileName: string; originalContent: string | null; newContent: string };
  return {
    type: 'diff',
    path: fileDiff.fileName,
    oldText: fileDiff.originalContent,
    newText: fileDiff.newContent,
  };
}
